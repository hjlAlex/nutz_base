...
public class VelocityResourceLoader extends ClasspathResourceLoader {
	...
	public InputStream getResourceStream(String name)
		...
		/**
		 * look for resource in thread classloader first (e.g. WEB-INF\lib in a
		 * servlet container) then fall back to the system classloader.
		 */

		try {
			result = ClassUtils.getResourceAsStream(getClass(), TEMPLATE_PATH
					+ name);
